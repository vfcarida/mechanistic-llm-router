...
class ModelCandidate(BaseModel):
    ...
    base_accuracy: float = Field(..., ge=0.0, le=1.0, description="Baseline capability benchmark accuracy.")
...
class ProbingSignals(BaseModel):
    ...
    fisher_j_norm: float = Field(..., description="Normalized Fisher J competence score in range [0, 1].")
    ...
    extra_metadata: dict[str, Any] = Field(default_factory=dict, description="Additional debug signals.")
...
class RoutingDecision(BaseModel):
    ...
    signals: dict[str, ProbingSignals] = Field(
        default_factory=dict, description="Topological probing signals per evaluated model candidate."
    )
